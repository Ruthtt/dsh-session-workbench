...
} from '../drag-contract.ts'
import type { SessionPointerDragDetail } from '../drag-contract.ts'
...
export interface RowDragProps {
  ...
  /** Workspace account used by pointer hit testing. */
  accountKey?: string | undefined
  /** Pointer path used for mouse, touchpad, pen, and touch drags. */
  pointer?: {
...
function SessionHoverContent({ node, now, t }: { node: SessionNode; now: number; t: RowTranslate }) {
  ...
  return (
    <div className={css.hoverContent}>
      ...
      <div className={css.hoverPath}>{t('hover.sessionId', { id: node.id })}</div>
...
export function SessionNodeItem({
  node, currentId, now, onOpen, onRename, onFork, onArchive, onExport, onDelete, drag, flat = false, t,
}: {
  node: SessionNode
  currentId: string | undefined
  now: number
  ...
  /** Download this session's exported log archive. */
  onExport: (id: SessionNode['id']) => void
  /** Open the browser-owned permanent-delete confirmation. */
  ...
  const emitPointerDrag = (detail: SessionPointerDragDetail): void => {
    window.dispatchEvent(new CustomEvent<SessionPointerDragDetail>(SESSION_POINTER_DRAG_EVENT, { detail }))
  }
  const onSessionPointerDown = (event: ReactPointerEvent<HTMLDivElement>): void => {
    if (drag === undefined || drag.pointer === undefined || event.button !== 0 || !event.isPrimary) return
    ...
    const finish = (phase: 'drop' | 'cancel'): void => {
      cleanup()
      if (!started) return
      setPointerPosition(null)
      emitPointerDrag({ phase, sessionId: node.id, clientX: lastX, clientY: lastY })
      if (phase === 'drop') pointer.drop(lastX, lastY)
      else pointer.cancel()
      suppressTimer = setTimeout(() => { suppressClickRef.current = false }, 0)
    }
    const onMove = (moveEvent: PointerEvent): void => {
      ...
      setPointerPosition({ x: lastX, y: lastY })
      emitPointerDrag({ phase: 'move', sessionId: node.id, clientX: lastX, clientY: lastY })
  ...
  const sessionMenuItems = [
    ...
    { id: 'export', label: t('menu.exportSession'), icon: <IconDownloadOutline16 /> },
  ...
  const ownRow = (
    <div
      className={clsx(
        ...
        pointerPosition !== null && css.pointerDragging,
        flat && !showStatus && css.flatSessionRowWithoutStatus,
        drag?.marker === 'before' && css.dropBefore, drag?.marker === 'after' && css.dropAfter,
      )}
      role="treeitem"
      aria-selected={selected}
      data-session-drop-id={drag === undefined ? undefined : node.id}
      data-session-drop-account={drag?.accountKey}
      data-pointer-draggable={drag?.pointer === undefined ? undefined : ''}
      onClick={(event) => {
      ...
      draggable={drag !== undefined && drag.pointer === undefined}
      onDragStart={drag === undefined
        ? undefined
        : (e) => {
    ...
    >
      ...
      {/* A blank New Session row is a provisional placeholder: nothing has
          ...
          (rename/fork/export/archive/delete) would all act on content that does not
      ...
      {pointerPosition !== null && (
        <div
          className={css.sessionDragPreview}
          style={{ left: pointerPosition.x + 14, top: pointerPosition.y + 14 }}
          aria-hidden="true"
        >
          {title}
        </div>
