...
interface DragState {
  ...
  /** Destination account, row marker, and insertion half. */
  over: { accountKey: string; id?: SessionNode['id']; half: 'before' | 'after' } | null
}
...
function pointerSessionDropAt(clientX: number, clientY: number): NonNullable<DragState['over']> | undefined {
  const hit = document.elementFromPoint(clientX, clientY)
  if (hit === null) return undefined
  const row = hit.closest<HTMLElement>('[data-session-drop-id][data-session-drop-account]')
  if (row !== null) {
    const accountKey = row.dataset.sessionDropAccount
    const id = row.dataset.sessionDropId
    if (accountKey === undefined || id === undefined) return undefined
    const rect = row.getBoundingClientRect()
    return {
      accountKey,
      id: id as SessionNode['id'],
      half: clientY < rect.top + rect.height / 2 ? 'before' : 'after',
    }
  }
  const group = hit.closest<HTMLElement>('[data-session-drop-group]')
  const accountKey = group?.dataset.sessionDropGroup
  return accountKey === undefined ? undefined : { accountKey, half: 'before' }
}

type SessionTreeProps = Pick<
  WorkspaceBrowserProps,
  'useSessions' | 'startSession' | 'open' | 'forkSession'
  | 'insertWorkspaceBefore' | 'insertSessionBefore' | 'migrateSession' | 't'
> & {
  ...
  /** Export a session log archive. */
  onSessionExport: (sessionId: SessionNode['id']) => void
  /** Open the permanent session-delete confirmation. */
...
function SessionTree({
  ...
  onRenameRequest, onDeleteRequest, onSessionRename, onSessionArchive, onSessionExport, onSessionDelete,
  insertWorkspaceBefore, insertSessionBefore, migrateSession, orderBy,
  ...
  // Native drag events can cross rows before React commits the drag-start
  // render. Keep the event-facing value synchronous so a quick drop is not
  // discarded by handlers that still close over the previous render.
  ...
  const commitSessionDrag = (activeDrag: DragState, over: NonNullable<DragState['over']>): void => {
    ...
    const targetIndex = over.id === undefined
      ? -1
      : targetGroup.sessions.findIndex(session => session.id === over.id)
    if (over.id !== undefined && targetIndex === -1) return
    const anchor = over.id === undefined
      ? targetGroup.sessions[0]?.id
      : over.half === 'before' ? over.id : targetGroup.sessions[targetIndex + 1]?.id
    if (activeDrag.accountKey !== over.accountKey) {
      if (targetGroup.workspaceId === undefined) return
      migrateSession(activeDrag.sessionId, targetGroup.workspaceId).then((childId) => {
        if (orderBy === 'manual' && anchor !== undefined) {
          return insertSessionBefore(targetGroup.workspaceId as WorkspaceId, childId, anchor)
        }
    ...
    const anchorIndex = anchor === undefined
      ...
      : targetGroup.sessions.findIndex(session => session.id === anchor)
          ...
          // (WorkspaceBrowser.module.css).
            <div
              ...
              className={clsx(
                ...
                drag?.over?.accountKey === group.key && drag.over.id === undefined && css.sessionGroupDrop,
              )}
              onDragOver={(e) => {
                if (workspaceDrag !== null && hoverWorkspace !== undefined) {
                  e.preventDefault()
                ...
                if (activeDrag === null || workspaceId === undefined || activeDrag.accountKey === group.key) return
                e.preventDefault()
                e.dataTransfer.dropEffect = 'move'
                const next: DragState = { ...activeDrag, over: { accountKey: group.key, half: 'before' } }
                dragRef.current = next
                setDrag(next)
              }}
              onDrop={(e) => {
                if (workspaceDrag !== null && dropWorkspace !== undefined) {
                  e.preventDefault()
                ...
                if (activeDrag === null || workspaceId === undefined || activeDrag.accountKey === group.key) return
                e.preventDefault()
                commitSessionDrag(activeDrag, { accountKey: group.key, half: 'before' })
              }}
            >
              <ProjectRowItem
                group={group}
              ...
              {(expandedSessionGroups.includes(group.key)
                ...
                const compatibleDrag = drag !== null
                  && (drag.accountKey === group.key || group.workspaceId !== undefined)
                const marker = compatibleDrag
                  && drag.over?.accountKey === group.key
                  && drag.over.id === node.id
                  ? drag.over.half
                  : null
                const dragProps = {
                  ...
                  start: () => {
                    ...
                    const next: DragState = { accountKey: group.key, sessionId: node.id, over: null }
                    dragRef.current = next
                    setDrag(next)
                  },
                  ...
                  /* v8 ignore next -- narrowing guard: Rows gates hover on `active`, which is false while the drag state is null. */
                    ...
                    const next: DragState = { ...activeDrag, over: { accountKey: group.key, id: node.id, half } }
                    dragRef.current = next
                    setDrag(next)
                  },
                  ...
                  /* v8 ignore next -- narrowing guard: Rows gates drop on `active`, which is false while the drag state is null. */
                    const activeDrag = dragRef.current
                    if (activeDrag === null) return
                    commitSessionDrag(activeDrag, { accountKey: group.key, id: node.id, half })
                  },
                  end: () => {
                    dragRef.current = null
                    setDrag(null)
                  ...
                  pointer: {
                    move: (clientX: number, clientY: number) => {
                      ...
                      const targetGroup = over === undefined
                        ? undefined
                        : groups.find(candidate => candidate.key === over.accountKey)
                      const compatible = over !== undefined && targetGroup !== undefined
                        && (activeDrag.accountKey === over.accountKey || targetGroup.workspaceId !== undefined)
                      const next: DragState = { ...activeDrag, over: compatible ? over : null }
                      dragRef.current = next
                      setDrag(next)
                    },
                    drop: (clientX: number, clientY: number) => {
                      ...
                      const targetGroup = over === undefined
                        ? undefined
                        : groups.find(candidate => candidate.key === over.accountKey)
                      if (activeDrag !== null && over !== undefined && targetGroup !== undefined
                        && (activeDrag.accountKey === over.accountKey || targetGroup.workspaceId !== undefined)) {
...
function FlatList({
  useSessions, open, forkSession, onSessionRename, onSessionArchive, onSessionExport, onSessionDelete, archivedSessionIds,
              ...
              drag={{
                ...
                hover: (half) => {
                  setDrag(current => current === null ? current : {
                    ...current,
                    over: { accountKey: FLAT_SESSION_ORDER_KEY, id: node.id, half },
                  })
                },
                drop: (half) => {
                  if (drag !== null) commitDrag(drag, {
                    accountKey: FLAT_SESSION_ORDER_KEY, id: node.id, half,
                  })
                },
                end: () => {
                  setDrag(null)
          ...
          : groupBy === 'flat'
            ? (
              <FlatList
                ...
                onSessionExport={exportSession} onSessionDelete={onSessionDelete}
                archivedSessionIds={archivedSessionIds}
                orderBy={orderBy}
